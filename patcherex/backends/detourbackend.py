...
class DetourBackend(Backend):
    ...
    def setup_headers(self, segments):
        ...
        if self.binary_type == "CGC":
            ...
            # if there is no phdr segment, then we record necessary information
            # so that we can create the segment in the future
    ...
    def set_added_segment_headers(self, nsegments):
        if self.binary_type == "CGC":
            ...
            code_segment_header = Container(**{"p_type": 1,
                                               "p_offset": self.added_code_file_start,
                                               "p_vaddr": self.mem_code_location,
                                               "p_paddr": self.mem_code_location,
            ...
            phdr_segment_header = Container(**{"p_type": 1,
                                               ...
                                               "p_align": 0x1000})
            self.ncontent = utils.bytes_overwrite(self.ncontent,
                                                  self.structs.Elf_Phdr.build(phdr_segment_header),
