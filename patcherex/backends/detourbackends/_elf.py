...
class DetourBackendElf(Backend):
    ...
    def __init__(self, filename, base_address=None, try_reuse_unused_space=False, replace_note_segment=False, try_without_cfg=False):
        super().__init__(filename, project_options={
            "main_opts": {"base_addr": base_address}})

        self.elf = ELFFile(open(filename, "rb"))
        # dump_segments also set self.structs
        ...
        self.ordered_nodes = self._get_ordered_nodes(
            self.cfg) if not self.try_without_cfg else None
        ...
        self.added_code_segment = int(
            (self.find_first_available_address() + 0x10000 - 1) / 0x10000) * 0x10000
        ...
        self.to_init_data = b""
        self.max_convertible_address = (
            1 << (32 if self.structs.elfclass == 32 else 48)) - 1
        ...
        self.patch_info = {"regions": {"original": [], "patched": []}, "new_segments": [
        ], "function_starts": {"original": [], "patched": []}}
    ...
    def find_first_available_address(self):
        load_segments = [segment for segment in self.elf.iter_segments(
        ) if segment["p_type"] == "PT_LOAD"]
        sorted_segments = sorted(
            load_segments, key=lambda x: x['p_vaddr'] + x['p_memsz'])
    ...
    def find_space(self):
        ...
        alloc_sections = [section for section in self.elf.iter_sections(
        ) if section['sh_flags'] & SH_FLAGS.SHF_ALLOC]
        sorted_sections = sorted(
            alloc_sections, key=lambda x: (x['sh_offset'], x['sh_size']))
        ...
        self.loaded_free_space = sorted(
            [space for space in self.free_space if space['type'] == "loaded"], key=lambda x: -x['file_size'])
        ...
        if self.try_reuse_unused_space:
            if len(self.added_code) > 0:
                if len(self.added_code) < self.reuse_code["file_size"]:
                    ...
                    l.info("Reusing space for code: 0x%x",
            ...
            if len(self.added_data) > 0:
                if len(self.added_data) < self.reuse_data["file_size"]:
                    ...
                    l.info("Reusing space for data: 0x%x",
            ...
            for segment in segments:
                if segment["p_type"] == "PT_NOTE":
                    ...
                    self.ncontent = utils.bytes_overwrite(
                        self.ncontent, self.structs.Elf_Phdr.build(segment), note_segment_header_loc)
                    break
            ...
            for segment in segments:
                if segment["p_type"] == "PT_LOAD":
                    ...
                    load_segments_rounded.append((
                        ...
                        ((segment["p_vaddr"] - \
                         self.first_load["p_vaddr"]) % 0x1000),
            ...
            for segment in segments:
                if segment["p_type"] == "PT_PHDR":
                    ...
                    load_segments_rounded = sorted(
                        load_segments_rounded, key=lambda x: x[0])
                    ...
                    while True:
                        ...
                        while i < len(load_segments_rounded) - 1:
                            ...
                            if prev_seg[1] > next_seg[0]:  # two segments overlap
                                ...
                                new_load_segments_rounded.append(prev_seg)
                                i += 1
                    ...
                    for prev_seg, next_seg in zip(load_segments_rounded[:-1], load_segments_rounded[1:]):
                        # round up to 0x1000
                        potential_base = (
                            (max(prev_seg[1], len(self.ncontent)) + 0xfff) & ~0xfff)
                        ...
                        self.phdr_start = load_segments_rounded[-1][1]
                    ...
                    segment["p_vaddr"] = self.phdr_start + \
                        self.first_load["p_vaddr"]
                    segment["p_paddr"] = self.phdr_start + \
                        self.first_load["p_vaddr"]
            ...
            self.ncontent = utils.bytes_overwrite(
                self.ncontent, self.patched_tag, old_phoff)
    ...
    def compile_c(self, code, optimization='-Oz', compiler_flags=""):
        ...
        with utils.tempdir() as td:
            ...
            res = utils.exec_cmd(
                "objcopy -B i386 -O binary -j .text %s %s" % (object_fname, bin_fname), shell=True)
            if res[2] != 0:
                print("objcopy error:")
                print(res[0])
                print(res[1])
