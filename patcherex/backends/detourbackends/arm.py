...
class DetourBackendArm(DetourBackendElf):
    ...
    def apply_patches(self, patches):
        ...
        if self.try_reuse_unused_space:
            self.name_map.force_insert(
                "ADDED_DATA_START", self.reuse_data['mem_start'])
        ...
        if self.try_reuse_unused_space:
            self.name_map.force_insert(
                "ADDED_CODE_START", self.reuse_code['mem_start'])
            ...
            if isinstance(patch, ReplaceFunctionPatch):
                ...
                if patch.size >= len(new_code):
                    ...
                    self.ncontent = utils.bytes_overwrite(
                        self.ncontent, new_code, file_offset)
                    l.info("Added patch: %s", str(patch))
                    self.patch_info["function_starts"]["original"].append(
                        patch.addr)
                    self.patch_info["function_starts"]["patched"].append(
                        patch.addr)
                else:
                    ...
                    l.info("Added patch: %s, @0x%x",
                           str(patch), detour_pos + offset)
                    self.patch_info["function_starts"]["original"].append(
                        patch.addr)
                    self.patch_info["function_starts"]["patched"].append(
                        detour_pos + offset + 1 if is_thumb else 0)
                self.added_patches.append(patch)
        ...
        self.patch_info["regions"]["patched"].append(
            [self.name_map["ADDED_CODE_START"], self.name_map["ADDED_CODE_START"] + len(self.added_code)])
    ...
    def get_movable_instructions(self, block):
        ...
        instructions = self.disassemble(
            original_bbcode, block.addr, is_thumb=is_thumb)
    ...
    def find_detour_pos(self, block, detour_size, patch_addr):
        ...
        possible_detour_ends = [(movable_bb_start + movable_bb_size if i.address ==
                                 movable_bb_start else i.address) - 1 for i in movable_instructions]
    ...
    def insert_detour(self, patch):
        ...
        mem = self.read_mem_from_file(
            block_addr if block_addr % 2 == 0 else block_addr - 1, self.project.factory.block(block_addr).size)
        ...
        for i in movable_instructions:
            if i.overwritten != "out":
                ...
                self.patch_bin(i.address, thumb_nop if self.check_if_thumb(
                    i.address) else arm_nop)

        # insert the jump detour
        offset = self.project.loader.main_object.mapped_base if self.project.loader.main_object.pic else 0
        detour_jmp_code = self.compile_jmp(detour_pos, self.get_current_code_position(
        ) + offset, is_thumb=self.check_if_thumb(detour_pos))
        ...
        patched_bbinstructions = self.disassemble(
            patched_bbcode, block_addr, is_thumb=self.check_if_thumb(block_addr))
        ...
        new_code = self.compile_moved_injected_code(
            movable_instructions, patch.code, offset=offset, is_thumb=self.check_if_thumb(patch.addr))
    ...
    def compile_function(self, code, compiler_flags="", is_thumb=False, entry=0x0, symbols=None):
        with utils.tempdir() as td:
            ...
            reassembled = self.compile_asm(
                disasm_str, base=entry, name_map={}, is_thumb=is_thumb)
