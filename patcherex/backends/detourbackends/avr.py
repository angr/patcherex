...
class DetourBackendAVR(DetourBackendElf):
    ...
    def __init__(self, filename, base_address=None, try_reuse_unused_space=False, replace_note_segment=False, try_without_cfg=False):
        ...
        self.name_map.force_insert(
            "ADDED_DATA_START", self.bss_section_addr + self.bss_section_size)
    ...
    def apply_patches(self, patches):
        ...
        relevant_patches = [p for p in patches if isinstance(
            p, (AddCodePatch, InsertCodePatch))]
        all_code = ""
        ...
        for patch in patches:
            if isinstance(patch, RemoveInstructionPatch):
                if patch.ins_size is None:
                    ...
                    size = patch.ins_size
                self.patch_bin(patch.ins_addr, b"\x00\x00" *
                               int((size + 2 - 1) / 2))
                self.added_patches.append(patch)
                l.info("Added patch: %s", str(patch))
        ...
        self.added_code_file_start = self.added_data_file_start + \
            len(self.added_data)
        self.name_map.force_insert("ADDED_CODE_START", self.added_code_file_start - (
        ...
        if len(self.added_data) > 0:
            ...
            data_load_start = self.name_map["ADDED_CODE_START"] - \
                len(self.added_data)
            ...
            patches.insert(0, InsertCodePatch(
                0x8c, code=do_copy_data_code, name="__do_copy_data", priority=1000))
    ...
    def disassemble(code, offset=0x0):  # pylint: disable=arguments-differ
        with utils.tempdir() as td:
            ...
            fp.close()
            res = utils.exec_cmd(
                "avr-objdump -D -b binary -m avr5 %s | tail +8" % (bin_fname), shell=True)
            if res[2] != 0:
        ...
        for line in str_result.splitlines():
            m = re.match(
                r"\s+(?P<address>[0-9a-f]+):\s+(?P<bytes>([0-9a-f]{2}\s)+)\s+(?P<mnemonic>.+?)\s+(?P<op_str>.+?)$", line)
            if m:
                ...
                instr['op_str'] = re.sub(
                    r'\s+', '', instr['op_str'].split(";")[0]).replace(",", ", ")
                result.append(instr)
        return result
    ...
    def compile_asm(code, name_map=None):  # pylint: disable=arguments-differ
        ...
        with utils.tempdir() as td:
            ...
            res = utils.exec_cmd("avr-as -mmcu=avr5 %s -o %s" %
                                 (asm_fname, object_fname), shell=True)
            if res[2] != 0:
