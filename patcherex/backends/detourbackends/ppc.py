...
class DetourBackendPpc(DetourBackendElf):
    ...
    def __init__(self, filename, base_address=None, try_reuse_unused_space=False, replace_note_segment=False, try_without_cfg=False):
        ...
        self.name_map.update(ADDED_DATA_START=(
    ...
    def set_oep(self, new_oep):
        if self.structs.elfclass == 64 and not self.structs.little_endian:
            current_hdr = self.structs.Elf_Ehdr.parse(self.ncontent)
            self.patch_bin(
                current_hdr["e_entry"], b"\x00\x00\x00\x00" + new_oep.to_bytes(4, 'big'))
        else:
        ...
        for patch in patches:
            if isinstance(patch, RemoveInstructionPatch):
                if patch.ins_size is None:
                    ...
                    size = patch.ins_size
                self.patch_bin(
                    patch.ins_addr, b"\x60\x00\x00\x00" * int((size + 4 - 1) / 4))
                self.added_patches.append(patch)
                l.info("Added patch: %s", str(patch))
            ...
            if isinstance(patch, ReplaceFunctionPatch):
                ...
                for k, v in self.name_map.items():
                    if k.startswith("_RFP" + str(patches.index(patch))):
                        patch.symbols[k[len(
                            "_RFP" + str(patches.index(patch))):]] = v
                new_code = self.compile_function(patch.asm_code, bits=self.structs.elfclass,
                                                 little_endian=self.structs.little_endian, entry=patch.addr, symbols=patch.symbols)
                file_offset = self.project.loader.main_object.addr_to_offset(
                    patch.addr)
                self.ncontent = utils.bytes_overwrite(
                    self.ncontent, b"\x60\x00\x00\x00" * (patch.size // 4), file_offset)
                if(patch.size >= len(new_code)):
                    ...
                    new_code = self.compile_function(patch.asm_code, bits=self.structs.elfclass,
                                                     little_endian=self.structs.little_endian, entry=detour_pos + offset, symbols=patch.symbols)
                    self.added_code += new_code
                    self.ncontent = utils.bytes_overwrite(
                        self.ncontent, new_code)
                    # compile jmp
    ...
    def insert_detour(self, patch):
        ...
        if self.try_without_cfg:
            ...
            detour_jmp_code = self.compile_jmp(
                patch.addr, self.get_current_code_position() + offset)
            patched_bbcode = self.read_mem_from_file(patch.addr, detour_size)
            patched_bbinstructions = self.disassemble(
                patched_bbcode, patch.addr)
            ...
            new_code = self.compile_asm(patch.code + "\n" + "\n".join([self.capstone_to_asm(s) for s in patched_bbinstructions]) + "\nb %s" % hex(
                patch.addr + 4 - offset), base=self.get_current_code_position(), name_map=self.name_map)
            return new_code
    ...
    def compile_asm(self, code, base=None, name_map=None):  # pylint: disable=arguments-differ
        # remvoe "r" before register
        code = re.subn(r'\br(\d+)', r"\1", code)[0]
    ...
    def compile_function(code, compiler_flags="", bits=32, little_endian=False, entry=0x0, symbols=None, data_only=False, prefix=""):
        with utils.tempdir() as td:
            ...
            with open(c_fname, 'w') as fp:
                fp.write(code)
            target = ("powerpcle-linux-gnu" if little_endian else "powerpc-linux-gnu") if bits == 32 else (
                "powerpc64le-linux-gnu" if little_endian else "powerpc64-linux-gnu")
            ...
            # Load Object File
            ld = cle.Loader(object2_fname, main_opts={
                            "base_addr": 0x0}, perform_relocations=False)
                ...
                if rodata_sym_index_new != -1 and rodata_sec_index != -1 and rodata_sym_index_old != -1:
                    for i in range(elf.num_sections()):
                        if elf.get_section(i).header['sh_name'] == symtab_section.header['sh_name']:
                            ...
                            content = f.read()
                            f.seek(
                                symtab_section['sh_offset'] + rodata_sym_index_new * symtab_section['sh_entsize'])
                            rodata_sym_new = f.read(
                                symtab_section['sh_entsize'])
                            content = utils.bytes_overwrite(
                                content, rodata_sym_new, symtab_section['sh_offset'] + rodata_sym_index_old * symtab_section['sh_entsize'])
                            f.seek(0)
                            f.write(content)
                            f.truncate()
                            break
                        ...
                        if rela_section.get_relocation(i)['r_info_type'] == 18:
                            ...
                            for j in range(elf.num_sections()):
                                if elf.get_section(j).header['sh_name'] == rela_section.header['sh_name']:
                                    f.seek(0)
                                    content = f.read()
                                    content = utils.bytes_overwrite(content, elf.structs.Elf_Rela.build(
                                        reloc), rela_section['sh_offset'] + i * rela_section['sh_entsize'])
                                    f.seek(0)
                                    f.write(content)
                                    f.truncate()
                                    break
            ...
            ld = cle.Loader(object2_fname, main_opts={
                            "base_addr": 0x0, "entry_point": 0x0})
            if data_only:
                patches = []
                for section in ld.all_objects[0].sections:
                    if section.name == ".rodata":
                        res = utils.exec_cmd("objcopy -B i386 -O binary -j %s %s %s" %
                                             (section.name, object2_fname, data_fname), shell=True)
                        if res[2] != 0:
                            raise ObjcopyException(
                                "Objcopy Error: " + str(res[0] + res[1], 'utf-8'))
                        with open(data_fname, "rb") as fp:
                            patches.append(AddRODataPatch(
                                fp.read(), name=prefix + section.name))
                        break
                return patches
            else:
                compiled = ld.memory.load(
                    ld.all_objects[0].entry, text_section_size)
                return compiled
